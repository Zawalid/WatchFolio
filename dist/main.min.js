...
`,u=async e=>{try{s.innerHTML=`
  ...
  `;else{s.innerHTML="";for(const o of t){var n=o.show,r=a(n);s.innerHTML+=r}}}catch{s.innerHTML=d}},h=e=>{s.classList.replace("hidden","flex"),s.scrollIntoView({behavior:"smooth"}),u(l.value)},f=()=>{""===l.value&&(s.classList.replace("grid","hidden"),s.innerHTML="",l.blur())},m=([h,f].forEach(t=>{l.addEventListener("keyup",e=>{"Enter"===e.key&&t()}),c.addEventListener("click",t)}),document.querySelector("#explore #shows")),x=document.querySelectorAll("#exploreTabs button"),g=document.querySelector("#explore #pagination"),p=async(e,t=null)=>{try{m.innerHTML=`
    <i
    class="fa-solid fa-spinner absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 animate-spin text-4xl text-thirdAccent"
  ></i>
    `;var n=await(await fetch("https://api.themoviedb.org/3/"+e+(t?"?page="+t:""),i)).json(),r=await Promise.all(n.results.map(async e=>{try{var t=await(await fetch("https://api.tvmaze.com/singlesearch/shows?q="+e.name)).json();if(t?.name&&!t.genres.includes("Anime"))return a(t)}catch{}}));m.innerHTML=r.join("")}catch{m.innerHTML=d}},v=(p("trending/tv/day"),x.forEach(e=>{e.addEventListener("click",function(){x.forEach(e=>e.classList.remove("active")),this.classList.add("active");var e=this.dataset.url;"trending"===this.dataset.name?g.classList.replace("flex","hidden"):(g.classList.replace("hidden","flex"),g.querySelectorAll("#numbers button").forEach((e,t)=>e.innerText=t+1),y(g.querySelectorAll("#numbers button")[0]),b=1,w(1,[v[0],v[1]])),p(e)})}),document.querySelectorAll("#pagination button"));let b=+g.dataset.current_page;const y=e=>{g.querySelectorAll("#numbers button").forEach(e=>e.classList.remove("active")),e.classList.add("active")},w=(e,t)=>{b===e?t.forEach(e=>e.style.opacity=".5"):t.forEach(e=>e.style.opacity="1")};function T(){google.accounts.id.initialize({client_id:"338374125620-cd7p79tha3ehscal53eonfjt8n2m5gud.apps.googleusercontent.com",callback:L}),google.accounts.id.prompt()}function L(e){e.credential&&(e=e.credential,r(n,o.credential(e)))}w(1,[v[0],v[1]]),g.addEventListener("click",async function(e){if(e.target.closest("button")){var e=e.target.closest("button"),t=document.querySelector("#exploreTabs .active").dataset.url,n=[...g.querySelectorAll("#numbers button")],r=await(await fetch("https://api.themoviedb.org/3/"+t,i)).json();const o=500<r.total_pages?500:r.total_pages;if("first"===e.id){if(1===b)return;b=1,n.forEach((e,t)=>e.innerText=t+1)}else if("last"===e.id){if(b===o)return;b=o,n.forEach((e,t,n)=>n.toReversed()[t].innerText=o-t)}else if("prev"===e.id){if(1===b)return;b--,n.find(e=>1==+e.innerText)||n.forEach(e=>e.innerText--)}else if("next"===e.id){if(b===o)return;b++,n.find(e=>+e.innerText===o)||n.forEach(e=>e.innerText++)}else b=+e.innerText,1!=+e.innerText&&+e.innerText!==o&&(2<(r=n.indexOf(e))&&!n.find(e=>+e.innerText===o)?(n[0].remove(),document.getElementById("numbers").insertAdjacentHTML("beforeend",`
        <button
      ...
      >
        ...
        `)));w(1,[v[0],v[1]]),w(o,[v[7],v[8]]),y(n.find(e=>+e.innerText===b)),p(t,b)}}),"serviceWorker"in navigator&&navigator.serviceWorker.register("./service-worker.js").then(e=>{}).catch(e=>{console.log("Service Worker Failed to Register",e)}),t(),window.onload=function(){T()},e(),document.getElementById("nav").querySelectorAll("li")[1].addEventListener("click",()=>{document.querySelector(".show")?.classList.remove("show"),document.body.classList.remove("overflow-hidden")});